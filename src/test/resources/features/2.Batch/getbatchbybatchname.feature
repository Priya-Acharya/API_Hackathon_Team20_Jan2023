...
Scenario Outline: Get Batch by Batch Name
    Given Batch with name "<batch_name>", description "<batch_description>", status "<batch_status>" and batchNoOfClasses "<batchNoOfClasses>" exists
    When The user makes a GET request for the Batch by Batch Name
    Then The returned Batch has name as "<batch_name>", description as "<batch_description>" and status as "<batch_status>"
